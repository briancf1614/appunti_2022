...
namespace NN_EsercitazioneIntermedia
{
    class ResponsabileVenditori:Venditore
    {
        ...
        public ResponsabileVenditori(List<Venditore> venditori)
        {
            Venditori = venditori;
        }
        ...
        {
            return $"{{{nameof(Venditori)}={Venditori}, {nameof(Settore)}={Settore.ToString()}, {nameof(Nome)}={Nome}, {nameof(Cognome)}={Cognome}, {nameof(Stipendio)}={Stipendio.ToString()}, Tredicesima: {Tredicesima()}}}";
        }
    }
}
